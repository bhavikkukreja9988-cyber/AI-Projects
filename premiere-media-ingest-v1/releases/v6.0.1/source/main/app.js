(function(){
  ...
  // Single Clip vs Batch — a sub-choice within Import mode only.
  function switchTab(tab) {
    ...
    if (tab === 'single') {
      ...
      $('tabSingle').className = 'subTab active';
      $('tabBatch').className = 'subTab';
      ...
      if($('test')) $('test').style.display = 'inline-block';
      $('tabSingle').className = 'subTab';
      $('tabBatch').className = 'subTab active';
      $('singleSection').style.display = 'none';
      $('batchSection').style.display = 'block';
    }
  }

  // Import vs History — the top-level mode switch. History gets the full
  // panel; none of the download/quality/insert controls render alongside it.
  function switchMode(mode) {
    activeMode = mode;
    if (mode === 'import') {
      $('modeImportBtn').className = 'segBtn active';
      $('modeHistoryBtn').className = 'segBtn';
      $('importModeSection').style.display = 'block';
      $('historySection').style.display = 'none';
      switchTab(activeTab);
    } else {
      $('modeImportBtn').className = 'segBtn';
      $('modeHistoryBtn').className = 'segBtn active';
  ...
  function toggleFold(rowId, bodyId) {
    var row = $(rowId), body = $(bodyId);
    if (!row || !body) return;
  ...
  function setManualTrack(on) {
    var sw = $('manualTrackSwitch'), fields = $('trackFields');
    if (!sw || !fields) return;
    sw.classList.toggle('on', !!on);
    fields.classList.toggle('open', !!on);
    if (!on) { $('vtrack').value = 'auto'; $('atrack').value = 'auto'; }
  }
  function toggleManualTrack() {
    var sw = $('manualTrackSwitch');
    setManualTrack(!(sw && sw.classList.contains('on')));
    scheduleSave();
  }
  ...
  function loadSettings(){
    try {
      ...
      switchMode(s.activeMode === 'history' ? 'history' : 'import');
      ...
      // If a specific (non-Auto) track was saved, open the manual track
      // fields automatically so the active preference isn't hidden.
      if(($('vtrack').value !== 'auto') || ($('atrack').value !== 'auto')) setManualTrack(true);
      updateModeHint();
      updateRangeDuration();
    } catch(e){}
  }
  ...
  function saveSettings(){
    try {
      var s = {
        ...
        preferPreset: $('preferPreset') ? $('preferPreset').value : 'h264'
      };
  ...
  async function detectTools() {
    ...
    var allOk = !!ytPath && !!ffPath;
    var dot = $('statusDot'), pillText = $('statusPillText');
    if (dot) dot.className = 'dot ' + (allOk ? 'good' : 'bad');
    if (pillText) pillText.textContent = allOk ? 'Tools OK' : 'Tools missing';
    return allOk;
  }
  ...
  async function downloadFullVideo(url) {
    ...
    var presetVal = $('preferPreset') ? $('preferPreset').value : 'h264';
    var preferH264 = presetVal !== 'small';
    var preferSmaller = presetVal === 'small';
    var streamMode = $('streamMode').value;

    var fmt = buildFormatSelector(quality, preferH264, preferSmaller, streamMode);
    var args = ['--no-playlist','--newline','--paths',videoFolder,'-o','%(id)s - %(title)s.%(ext)s','--format-sort',fmt.sort,'--format-sort-force','-f',fmt.selector,'--merge-output-format','mp4','--print','after_move:filepath',url];
  ...
  async function run(){
    ...
    try{
      ...
      if (activeTab === 'batch') {
        ...
        setStageProgress(100); setProgress(100); setProgressDetail('Complete.');
        var diagBits = [];
        if (result.sequenceName) diagBits.push('Sequence: ' + result.sequenceName);
        if (typeof result.playheadRead === 'number') diagBits.push('Playhead read: ' + result.playheadRead.toFixed(2) + 's');
        var diagLine = diagBits.length ? ('\n' + diagBits.join(' • ')) : '';
        setStatus('Complete\nInserted ' + (result.totalInserted || item.ranges.length) + ' clip(s) on the timeline.' + diagLine, 'good');
  ...
  $('modeImportBtn').addEventListener('click', function(){ switchMode('import'); scheduleSave(); });
  $('modeHistoryBtn').addEventListener('click', function(){ switchMode('history'); scheduleSave(); });
  $('statusPill').addEventListener('click', toggleToolsPopover);
  $('foldQuality').addEventListener('click', function(){ toggleFold('foldQuality','foldQualityBody'); });
  $('foldInsert').addEventListener('click', function(){ toggleFold('foldInsert','foldInsertBody'); });
  $('foldAdvanced').addEventListener('click', function(){ toggleFold('foldAdvanced','foldAdvancedBody'); });
  $('manualTrackSwitch').addEventListener('click', toggleManualTrack);
  ...
  ['url','start','end','batchText','vtrack','atrack','streamMode','insertMode','scaleToFrame','snapToMarker','reuseExisting','quality','preferPreset','useTimeRange','importMode'].forEach(function(id){
    var el = $(id);
