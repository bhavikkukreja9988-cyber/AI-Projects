...
function mediaIngestFindStackedTrackAtPosition(seq, duration, position, preferredV, preferredA, streamMode) {
    ...
    var vOrder = [];
    if (includeVideo) {
        var vPreferredValid = (preferredV >= 0 && preferredV < maxV);
        if (vPreferredValid) vOrder.push(preferredV);
        for (var vv = 0; vv < maxV; vv++) {
            if (!(vPreferredValid && vv === preferredV)) vOrder.push(vv);
        }
    } else {
        vOrder = [-1];
    }
    var aOrderBase = [];
    if (includeAudio) {
        var aPreferredValid = (preferredA >= 0 && preferredA < maxA);
        if (aPreferredValid) aOrderBase.push(preferredA);
        for (var aa = 0; aa < maxA; aa++) {
            if (!(aPreferredValid && aa === preferredA)) aOrderBase.push(aa);
        }
    } else {
        aOrderBase = [-1];
    }
...
function mediaIngestDispatch(action, payloadString) {
    try {
        ...
        var seqNameForReport = '';
        try { seqNameForReport = String(seq.name || ''); } catch (eSeqName) {}
        ...
        return JSON.stringify({
            ...
            sequenceName: seqNameForReport,
            playheadRead: playhead
        });
    } catch (e) {
