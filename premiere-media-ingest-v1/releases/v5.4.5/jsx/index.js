...
function mediaIngestFindStackedTrackAtPosition(seq, duration, position, preferredV, preferredA, streamMode) {
    ...
    var startV = (preferredV >= 0 && includeVideo) ? preferredV : 0;
    var endV = includeVideo ? maxV : 1;
    ...
    for (var v = startV; v < endV; v++) {
        ...
        var startA = (preferredA >= 0) ? preferredA : 0;
        for (var a = startA; a < maxA; a++) {
        ...
        for (var i = 0; i < ranges.length; i++) {
            ...
            if (insertMode === 'stack') {
                ...
                if (!stacked) {
                    ...
                    if (!fallback) {
                        throw new Error('No free track space available at or after ' + currentPosition.toFixed(2) + 's.');
                    }
